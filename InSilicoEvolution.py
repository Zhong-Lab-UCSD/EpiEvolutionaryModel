...
def Evolution(n, par, base, base_p, trans_dic, imm_p, norm_p, norm_cut):
  ...
  if local:
    ...
    for i in xrange(n):
      ...
      for epi in xrange(1,epi_n+1):     
        seq_pair.S_epi[epi-1]+=numpy.random.choice(['0','1'], p=[1-phi_list1[epi-1], phi_list1[epi-1]] )
  else:    
    for i in xrange(n):
      seq_pair.S+=numpy.random.choice(base, p=base_p)
      for epi in xrange(1,epi_n+1):
        seq_pair.S_epi[epi-1]+=numpy.random.choice(['0','1'], p=par.equil_dict[str(epi)])

  if len(seq_pair.S_epi)==0:
    seq_pair.S_epi="?"*n
    seq_pair.flag=0
      ...
      if seq_pair.flag:
        for epi in xrange(1,epi_n+1):
          if hypN==1 or equ_f==1:
            seq_pair.Sd_epi[epi-1]+=numpy.random.choice(['0','1'], p=trans_dic[str(epi)+seq_pair.S_epi[epi-1][si]])
          else:
            seq_pair.Sd_epi[epi-1]+=numpy.random.choice(['0','1'], p=par.equil_dict[str(epi)])
